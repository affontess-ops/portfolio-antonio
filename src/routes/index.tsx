...
const campaigns: Campaign[] = [
  ...
  {
    name: "Indique um Amigo", intro: "Campanha de indicação com recompensa de R$20 por indicação aprovada, até 55 indicações por cliente, totalizando até R$1.100 acumuláveis.", strategy: "Estratégia de crescimento orgânico via prova social e recompensa mensurável, apresentando a mecânica de forma explícita para reduzir dúvidas na conversão e transformar o próprio usuário em canal de aquisição.", channels: ["Instagram", "WhatsApp", "Site", "App"], indicators: ["Cliques no CTA", "Compartilhamentos", "Indicações realizadas", "Conversões", "Novos usuários"], pieces: [
...
function Home() {
  return (
    <div id="portfolio-page" className="min-h-screen bg-background text-foreground overflow-x-hidden">
...
function EduBlock({ degree, uni, year, note }: { degree: string; uni: string; year: string; note?: string }) { return <div className="p-6 rounded-xl bg-surface border border-border"><div className="font-display font-bold text-xl">{degree}</div><div className="text-sm text-accent mt-1">{uni}</div><div className="text-xs text-muted-foreground mt-2">{year}</div>{note && <div className="text-xs text-muted-foreground mt-3">{note}</div>}</div>; }
